...
if (isset($nombre) && isset($ap1) && isset($mail) && isset($nic) && isset($pass) && isset($pass2)) {
    ...
    $error = false;
    if ($pass != $pass2) {
        echo '<h2 style="color:white;background-color:red;">Las contraseñas introducidas no coinciden.</h2>';
    }

    //Comprobamos que el correo tiene un formato correcto.
    if (!$error && ($mail = filter_var($mail, FILTER_VALIDATE_EMAIL)) === false) {
        echo '<h2 style="color:white;background-color:red;">Formato de email incorrecto</h2>';
    }

    if (!$error && alta_usuario($nombre, $ap1, $ap2, $mail, $nic, $pass, $pass2)) {
        echo '<h2 style="background-color:green;">Usuario registrado</h2>';
    } else {
        echo '<h2 style="color:white;background-color:red;">No se ha podido dar de alta al usuario.</h2>';
    }
}
?>
